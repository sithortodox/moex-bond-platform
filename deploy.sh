#!/bin/bash
set -e
echo "=== MOEX Bond Platform Deploy ==="
...
# 5. DB schema
echo "[5/8] Applying DB schema..."
sudo -u postgres psql -d moex_bonds -f /opt/moex-bond-platform/schema.sql

# 6. Systemd
echo "[6/8] Setting up systemd..."
cat > /etc/systemd/system/moex-bond-screener.service << 'SVC'
[Unit]
...
cat > /etc/nginx/sites-available/moex-bond-screener << 'NGX'
server {
    listen 80;
